...
object Sequences: // Essentially, generic linkedlists
  ...
  object Sequence:
    ...
    def evenIndices[A](s: Sequence[A]): Sequence[A] =
      def iter(s: Sequence[A], counter: Int): Sequence[A] = s match
        case Cons(h, t) if counter % 2 == 0 => Cons(h, iter(t, counter + 1))
        case Cons(h, t) if counter % 2 != 0 => iter(t, counter + 1)
        case _ => Nil()
      iter(s, 0)
    ...
    def contains[A](s: Sequence[A])(elem: A): Boolean = s match
      ...
      case Cons(h, t) if h != t => contains(t)(elem)
      case Nil() => false
      case _ => false
    ...
    def distinct[A](s: Sequence[A]): Sequence[A] =
      def iter(s: Sequence[A], acc: Sequence[A]): Sequence[A] = s match
        case Cons(h, t) if !contains(acc)(h) => Cons(h, iter(t, Cons(h, acc)))
        case Cons(h, t) if contains(acc)(h) => iter(t, acc)
        case _ => Nil()
      iter(s, Nil())
